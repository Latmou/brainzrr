...
export function RecordingItem({ recording, artistName, artistId, releaseId, index, showIndex = true, coverArtUrl, fullTracklist }: RecordingItemProps) {
  const { play, pause, isPlaying, currentTrack, setQueue, isLoading } = usePlayer()
  ...
  return (
    <div 
      className={cn(
        "flex items-center gap-4 p-2 rounded transition-colors group hover:bg-white/10 cursor-pointer"
      )}
      ...
      {showIndex && typeof index === 'number' && (
        <div className="w-4 flex items-center justify-end">
          {isCurrentTrack && isLoading ? (
            ...
            <>
              <div className={cn(
                "text-zinc-400 text-sm group-hover:hidden",
          ...
          {isCurrentTrack && isLoading ? (
            ...
            <>
              <Music size={20} className={cn(
                "text-zinc-500 group-hover:hidden",
                isCurrentTrack && "text-green-500"
              )} />
              <Play 
                size={15} 
